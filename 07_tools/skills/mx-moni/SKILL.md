...
not_for:
  ...
  - 商业用途、代他人操作、非法交易演示
# 环境变量配置
# Windows 本地环境建议优先通过以下方式检查用户级环境变量：
# [System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")
# [System.Environment]::GetEnvironmentVariable("MX_API_URL", "User")
parameters:
  - name: MX_APIKEY
...
- **Windows 本地检查方式**：在 PowerShell 中运行 `[System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")`；如需自定义地址，可用同样方式检查 `MX_API_URL`。
...
在使用前，请确保已配置以下环境变量，并先在 PowerShell 中检查：

```powershell
$mxApiKey = [System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")
$mxApiUrl = [System.Environment]::GetEnvironmentVariable("MX_API_URL", "User")

# 若 $mxApiUrl 为空，则默认使用 https://mkapi2.dfcfs.com/finskillshub
```

## 使用方式（直接 Python 脚本调用）

1. 在妙想Skills页面获取apikey。
2. 将apikey存到 Windows 用户级环境变量 `MX_APIKEY`，并使用 `[System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")` 检查本地 key 是否存在；若存在可直接用。
...
```
   ...
   > - **禁止硬编码**: 不要把 API Key 写入 `SKILL.md`、脚本、示例命令、提示词或聊天记录中。
...
- 用户需在妙想Skills页面获取并配置 `MX_APIKEY` 和 `MX_API_URL` 环境变量；在 Windows 上请优先用 `[System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")` 检查 key 是否已就绪。
