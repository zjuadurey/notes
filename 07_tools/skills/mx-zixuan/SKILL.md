...
- **Windows 本地检查方式**: 在 PowerShell 中运行 `[System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")`
...
2. 将apikey配置到 Windows 用户级环境变量 `MX_APIKEY`
3. 在 PowerShell 中运行 `[System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")` 确认可以取到值
4. 确保网络可以访问 `https://mkapi2.dfcfs.com`
...
```powershell
# 先确认 Windows 用户级环境变量中已经有 MX_APIKEY
...
## 错误处理
- 未配置apikey: 提示检查 Windows 用户级环境变量 `MX_APIKEY`，并用 `[System.Environment]::GetEnvironmentVariable("MX_APIKEY", "User")` 验证
- 接口调用失败: 显示错误信息
- 数据为空: 提示用户到东方财富App查询
